...
(function () {
    ...
    var USER_SYNC_BIDDERS = [
        'sovrn', 'appnexus', 'ix', 'rubicon',
        'underdogmedia', 'cadent_aperture_mx', 'sharethrough'
    ];
    ...
    googletag.cmd.push(function () {
        ...
        if (config.targeting) {
            Object.keys(config.targeting).forEach(function (key) {
                googletag.pubads().setTargeting(key, config.targeting[key]);
            });
        }
    ...
    function sendBidsAndRefresh(adUnitCodes) {
        googletag.cmd.push(function () {
            try {
            ...
            if (slotsToRefresh.length) {
                ...
                // If the slot is currently in-viewport (viewableSinceMs > 0)
                // restart the clock from now; if it left the viewport since
                // dispatch, it stays at 0 until the observer fires again.
                var now = Date.now();
